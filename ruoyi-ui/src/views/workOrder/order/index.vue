...
<!--      <el-table-column label="工单接收人ID" align="center" prop="orderReceiverId" />-->
    ...
    <el-dialog :title="title" :visible.sync="open" width="700px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="任务类型" prop="taskType">
              <el-select v-model="form.taskType" placeholder="请选择任务类型" @change="handleTaskConfigChange" filterable>
                <el-option
                  v-for="task in taskOptions"
                  :key="task.id"
                  :label="task.taskName"
          ...
          <el-col :span="12">
            <el-form-item label="任务数量" prop="taskNum">
              <el-input-number v-model="form.taskNum"  :min="1"  style="width: 220px;"  />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="积分数量" prop="integralNum">
              <el-input-number v-model="form.integralNum" placeholder="请输入积分数量" :min="0" :step="0.1" :precision="1" style="width: 220px;" disabled/>
            </el-form-item>
          </el-col>
          <!-- 任务开始时间 -->
          <el-col :span="12">
...
<!--          </el-col>-->
    ...
    <el-dialog :title="auditTitle" :visible.sync="openAudit" width="700px" append-to-body>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="auditSubmit" :loading="auditSubmitLoading">确 定</el-button>
...
<script>
import { listWorkOrder, getWorkOrder, delWorkOrder, addWorkOrder, updateWorkOrder,auditWorkOrder } from "@/api/workOrder/order";
...
export default {
  ...
  watch: {
    'form.taskNum': function(newVal) {
      if (this.form.taskType) {
        this.handleTaskConfigChange(this.form.taskType);
      }
    }
  },
  methods: {
    ...
    submitForm() {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          const taskConfig =this.taskOptions.find(item => item.id == this.form.taskType)
          this.form.taskName = taskConfig ? taskConfig.taskName : '';
    ...
    handleTaskConfigChange(value) {
      const selectedTask = this.taskOptions.find(task => task.id === value);
      if (selectedTask) {
        this.form.integralNum = selectedTask.integralNum*this.form.taskNum
      }
    }
  }
};
